...
## Q54. What is the useReducer hook and when is it preferred over useState?

useReducer is a React Hook. It is used to manage state in a more organized way. It is useful when the state has complex logic or when there are multiple ways to update the state.

I prefer useReducer over useState when the state has multiple related values or when the state can change in several different ways. For simple state, I prefer useState because it is easier and more straightforward.

## Q55. Explain the useMemo hook and give a use case.

useMemo is a React Hook. It is used for performance optimization. It is used to cache the result of an expensive calculation and reuse that result when the dependencies have not changed. This can help avoid doing the same calculation again during every render.

For example, if I have a large list of products and I need to filter or sort that list, I can use useMemo to avoid repeating that calculation when unrelated state changes.
